...
class Chess:
    ...
    def simulate_move(self, piece,row,col):
        ...
        print("Position initiale de la piece :",piece_row, piece_col)
        print("Position actuelle de la piece :",row,col)
    ...
    def select(self,row,col):
        ...
        if piece != 0 and self.turn == piece.color:
            ...
            print("Déplacements autorisés", self.valid_moves)
    def _move(self,row,col):
        piece = self.Board.get_piece(row,col)
        print("Piece selectionnee : ", self.selected.type)
    ...
    def remove(self,board,piece,row,col):
        ...
        print("Pieces blanches restantes : ", self.White_pieces_left)
        print("Pieces noires restantes : ", self.Black_pieces_left)
    def draw_available_moves(self):
        if len(self.valid_moves) > 0:
            for pos in self.valid_moves:
                row,col = pos[0],pos[1]
                pygame.draw.circle(self.Win, Chocolate, (col*self.Square + self.Square//2, row*self.Square + self.Square//2),self.Square//6)
